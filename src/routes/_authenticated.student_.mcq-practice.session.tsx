...
export const Route = createFileRoute("/_authenticated/student_/mcq-practice/session")({
  validateSearch: (s) => searchSchema.parse(s),
  head: () => ({
    meta: [
      { title: "Practice Session — MCQ Practice" },
...
function SessionPage() {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      await queryClient.invalidateQueries({ queryKey: ["mcq-practice", "taxonomy"] });
      navigate({
  ...
  if (!session) {
    return (
      <NotFoundShell
        ...
        onBack={() => navigate({ to: "/student/mcq-practice", search: {} })}
      />
    );
  }
...
function Session({
  ...
  const submitFn = useServerFn(submitMcqAnswer);
  const bookmarkFn = useServerFn(toggleMcqBookmark);
  ...
  const submitMutation = useMutation({
    ...
    onSuccess: () => {
      ...
      queryClient.invalidateQueries({ queryKey: ["mcq-practice", "taxonomy"] });
    },
  });
  ...
  const sendReport = (_reason: string) => {
    ...
    // no reports table for MCQs. The UI cue remains so students can flag.
    setReportedIdx((s) => {
      const next = new Set(s);
      next.add(cursor);
      return next;
