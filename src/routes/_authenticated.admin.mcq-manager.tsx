...
export const Route = createFileRoute("/_authenticated/admin/mcq-manager")({
  head: () => ({
    meta: [
      { title: "MCQ Manager — Admin Console" },
      { name: "robots", content: "noindex, nofollow" },
    ],
  }),
  component: McqManagerPage,
});
...
const STATUSES: McqStatus[] = ["draft", "review", "published", "archived"];
const PAGE_SIZES = [25, 50, 100, 250, 500] as const;
const PAGE_SIZE_KEY = "mcq_manager_page_size";
const FILTERS_KEY = "mcq_manager_filters_v2";
...
function McqManagerPage() {
  ...
  const [viewRow, setViewRow] = useState<McqRow | null>(null);
  const [editRow, setEditRow] = useState<McqRow | null>(null);
  const [moveRowsSel, setMoveRowsSel] = useState<McqRow[] | null>(null);
  const [deleteRowsSel, setDeleteRowsSel] = useState<McqRow[] | null>(null);

  /* -------- Server data -------- */

  const listFn = useServerFn(listMcqs);
  const statsFn = useServerFn(getMcqStats);
  const treeFn = useServerFn(getAcademicTree);
  const createFn = useServerFn(createMcq);
  const updateFn = useServerFn(updateMcq);
  const deleteFn = useServerFn(deleteMcqs);
  const statusFn = useServerFn(changeMcqStatus);
  const moveFn = useServerFn(moveMcqs);
  const importFn = useServerFn(bulkImportMcqs);
  ...
  const listParams = useMemo(
    () => ({
      ...
      status: (status || null) as McqStatus | null,
      batchId: batch || null,
  ...
  const list: McqListResult = listQuery.data ?? {
    rows: [],
    total: 0,
    page,
    pageSize,
  ...
  function invalidateAll() {
    qc.invalidateQueries({ queryKey: ["mcq-list"] });
    qc.invalidateQueries({ queryKey: ["mcq-stats"] });
  }
  ...
  const statusMut = useMutation({
    mutationFn: (payload: { ids: string[]; status: McqStatus }) => statusFn({ data: payload }),
    onSuccess: (res) => {
  ...
  const exportRows = useCallback((rows: McqRow[]) => {
    const header = [
      "id",
      "question",
      "A",
    ...
    a.href = url;
    a.download = `mcq-export-${new Date().toISOString().slice(0, 10)}.csv`;
    a.click();
...
function BulkActionBar({
  ...
  onDelete: () => void;
  onExport: () => void;
  onChangeStatus: (s: McqStatus) => void;
}) {
...
function TableCard(props: {
  ...
  onView: (r: McqRow) => void;
  onEdit: (r: McqRow) => void;
  onMove: (r: McqRow) => void;
  onDelete: (r: McqRow) => void;
  onOpenUpload: () => void;
  onResetFilters: () => void;
}) {
  const {
...
function StatusDot({ value }: { value: McqStatus }) {
  const cls =
    value === "published"
      ? "bg-success"
      : value === "review"
...
function StatusBadge({ value }: { value: McqStatus }) {
  const map: Record<McqStatus, { label: string; cls: string }> = {
...
function EmptyState({ onUpload, onReset }: { onUpload?: () => void; onReset?: () => void }) {
  return (
    <div className="relative flex flex-col items-center justify-center gap-4 px-6 py-16 text-center">
      ...
      <div>
        <div className="text-sm font-semibold text-foreground">No MCQs match these filters</div>
    ...
    <ModalShell onClose={onClose} maxWidth="max-w-3xl">
      ...
      <div className="flex items-center justify-end gap-2 border-t border-border/70 px-6 py-4">
        ...
        <button
          ...
          className="inline-flex h-9 items-center gap-1.5 rounded-xl bg-gradient-to-br from-primary to-accent px-3 text-xs font-semibold text-primary-foreground shadow transition hover:-translate-y-0.5"
        >
          <Pencil className="h-3.5 w-3.5" /> Edit MCQ
        </button>
      </div>
    </ModalShell>
  );
...
function EditModal({
  ...
  const [status, setStatus] = useState<McqStatus>(row.status);
  ...
  return (
    <ModalShell onClose={onClose} maxWidth="max-w-3xl">
      <ModalHeader icon={Pencil} title="Edit MCQ" subtitle={row.id.slice(0, 8)} onClose={onClose} />
      <div className="max-h-[70vh] space-y-5 overflow-auto px-6 py-5">
        ...
        <div className="grid grid-cols-1 gap-3 sm:grid-cols-4">
          ...
          <FormField label="Status">
            <IdSelectField
              value={status}
              onChange={(v) => setStatus(v as McqStatus)}
...
function MoveModal({
  ...
  return (
    <ModalShell onClose={onClose} maxWidth="max-w-lg">
      <ModalHeader
        icon={ArrowRightLeft}
        title={rows.length === 1 ? "Move MCQ" : `Move ${rows.length} MCQs`}
      ...
      <div className="flex items-center justify-end gap-2 border-t border-border/70 px-6 py-4">
        ...
        >
          ...
          {busy ? "Moving…" : `Move ${rows.length > 1 ? `${rows.length} MCQs` : "MCQ"}`}
        </button>
      </div>
    </ModalShell>
  );
...
function DeleteModal({
  ...
  return (
    <ModalShell onClose={onClose} maxWidth="max-w-lg">
      <ModalHeader
        icon={TriangleAlert}
        title={rows.length === 1 ? "Delete this MCQ?" : `Delete ${rows.length} MCQs?`}
      ...
      <div className="space-y-4 px-6 py-5">
        <div className="rounded-xl border border-destructive/30 bg-destructive/[0.06] p-3 text-xs text-destructive">
          <div className="flex items-center gap-2 font-semibold">
            ...
            You are about to permanently remove {rows.length.toLocaleString()} MCQ
            {rows.length > 1 ? "s" : ""}.
          </div>
