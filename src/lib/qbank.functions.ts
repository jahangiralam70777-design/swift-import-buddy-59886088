// Server functions for the Qns Bank Manager. Mirrors mcq.functions.ts exactly;
//
...
export type QBankStatus = "draft" | "review" | "published" | "archived";

export type QBankOption = { key: string; text: string };

export type QBankRow = {
  id: string;
  question: string;
  options: QBankOption[];
  answer: string;
...
// eslint-disable-next-line @typescript-eslint/no-explicit-any
function normalizeOptions(raw: any): QBankOption[] {
...
// ---------------------------------------------------------------------------
// listQuestionBankQuestions — server-side filter / sort / paginate
// ---------------------------------------------------------------------------

export type ListQBankInput = {
  page?: number;
  pageSize?: number;
  levelId?: string | null;
  subjectId?: string | null;
  chapterId?: string | null;
  status?: QBankStatus | null;
  batchId?: string | null;
  search?: string | null;
...
function validateListInput(input: unknown): ListQBankInput {
  ...
  const sort =
    ...
    ["newest", "oldest", "updated", "position", "question", "chapter"].includes(src.sort)
      ? (src.sort as ListQBankInput["sort"])
      : "newest";
  return {
    page,
    pageSize,
...
export const listQuestionBankQuestions = createServerFn({ method: "POST" })
  .middleware([requireSupabaseAuth])
  .inputValidator(validateListInput)
  .handler(async ({ data, context }): Promise<QBankListResult> => {
    ...
    let q = supabase.from("qbank_questions").select("*", { count: "exact" });
...
function emptyListResult(input: ListQBankInput, batches: string[]): QBankListResult {
  return {
    rows: [],
    total: 0,
    page: input.page!,
...
// ---------------------------------------------------------------------------
// createQuestionBankQuestion / updateQuestionBankQuestion
// ---------------------------------------------------------------------------
...
export const createQuestionBankQuestion = createServerFn({ method: "POST" })
  .middleware([requireSupabaseAuth])
...
export const updateQuestionBankQuestion = createServerFn({ method: "POST" })
  .middleware([requireSupabaseAuth])
...
// ---------------------------------------------------------------------------
// deleteQuestionBankQuestions (single or bulk)
// ---------------------------------------------------------------------------

export const deleteQuestionBankQuestions = createServerFn({ method: "POST" })
  .middleware([requireSupabaseAuth])
  .inputValidator((input: unknown) => {
    const src = (input ?? {}) as Record<string, unknown>;
  ...
  .handler(async ({ data, context }) => {
    await assertAdmin(context);
    const { error } = await context.supabase.from("qbank_questions").delete().in("id", data.ids);
    if (error) throw new Error(error.message);
    return { deleted: data.ids.length };
  });

// ---------------------------------------------------------------------------
// changeQuestionBankStatus (bulk)
// ---------------------------------------------------------------------------

export const changeQuestionBankStatus = createServerFn({ method: "POST" })
  .middleware([requireSupabaseAuth])
  .inputValidator((input: unknown) => {
    const src = (input ?? {}) as Record<string, unknown>;
    const ids = Array.isArray(src.ids) ? src.ids.filter((x) => typeof x === "string") : [];
    ...
    return { ids: ids as string[], status: status as QBankStatus };
  })
  .handler(async ({ data, context }) => {
    await assertAdmin(context);
    const { error } = await context.supabase
...
// ---------------------------------------------------------------------------
// moveQuestionBankQuestions — reassign chapter (bulk). Appends to end of target chapter.
// ---------------------------------------------------------------------------

export const moveQuestionBankQuestions = createServerFn({ method: "POST" })
  .middleware([requireSupabaseAuth])
  .inputValidator((input: unknown) => {
    const src = (input ?? {}) as Record<string, unknown>;
    const ids = Array.isArray(src.ids) ? src.ids.filter((x) => typeof x === "string") : [];
...
// ---------------------------------------------------------------------------
// bulkImportQuestionBankQuestions — chunked insert with duplicate detection.
//
...
export type QBankBulkImportResult = {
  inserted: number;
...
export const bulkImportQuestionBankQuestions = createServerFn({ method: "POST" })
  .middleware([requireSupabaseAuth])
  .inputValidator(validateBulkImport)
  .handler(async ({ data, context }): Promise<QBankBulkImportResult> => {
    await assertAdmin(context);
    const { supabase } = context;
    ...
    const existingSet = new Set(
      (existing ?? []).map((r: { question: string | null }) =>
        (r.question ?? "").trim().toLowerCase(),
      ),
    );
...
// ---------------------------------------------------------------------------
// getQuestionBankStats — KPI panel data
// ---------------------------------------------------------------------------

export const getQuestionBankStats = createServerFn({ method: "GET" })
  .middleware([requireSupabaseAuth])
