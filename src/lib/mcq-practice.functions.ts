// Server functions powering the Student MCQ Practice module.
...
// ---------------------------------------------------------------------------
// getMcqTaxonomy — full Level → Subject → Chapter tree with per-chapter
// totals and the caller's own progress counters, in one round-trip.
// ---------------------------------------------------------------------------

export const getMcqTaxonomy = createServerFn({ method: "GET" })
  .middleware([requireSupabaseAuth])
  .handler(async ({ context }): Promise<PracticeTaxonomy> => {
    const { supabase } = context;
    const { data, error } = await supabase.rpc("mcq_practice_taxonomy");
...
// ---------------------------------------------------------------------------
// getChapterSession — questions + user attempts + bookmarks for one chapter
// ---------------------------------------------------------------------------

export const getChapterSession = createServerFn({ method: "POST" })
  .middleware([requireSupabaseAuth])
...
// ---------------------------------------------------------------------------
// submitMcqAnswer — upsert attempt + track wrong answer + return outcome
// ---------------------------------------------------------------------------

export const submitMcqAnswer = createServerFn({ method: "POST" })
  .middleware([requireSupabaseAuth])
  .inputValidator((input: unknown) => {
    const src = (input ?? {}) as Record<string, unknown>;
  ...
  .handler(async ({ data, context }): Promise<{ isCorrect: boolean; correctIndex: number }> => {
    ...
    const { error: aErr } = await supabase.from("mcq_attempts").upsert(
      {
        user_id: userId,
...
// ---------------------------------------------------------------------------
// restartChapterMcqSession — wipe user's attempts for a chapter so the
// next session starts from scratch. Only touches the caller's own rows.
// ---------------------------------------------------------------------------

export const restartChapterMcqSession = createServerFn({ method: "POST" })
  .middleware([requireSupabaseAuth])
  .inputValidator((input: unknown) => {
    const src = (input ?? {}) as Record<string, unknown>;
    const chapterId = typeof src.chapterId === "string" ? src.chapterId : "";
