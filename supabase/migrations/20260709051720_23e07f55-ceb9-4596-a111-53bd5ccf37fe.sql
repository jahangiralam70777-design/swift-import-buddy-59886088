...
-- Admins can view all roles (added after has_role is defined)
...
-- =========================================================================
-- Admin helper: allow admin full access to a table
-- (defined as policy per table below)
-- =========================================================================
...
create policy "academic_levels read auth"
  on public.academic_levels for select
  to authenticated
  using (true);

create policy "academic_levels admin write"
...
create policy "academic_subjects read auth"
  on public.academic_subjects for select to authenticated using (true);

create policy "academic_subjects admin write"
...
create policy "academic_chapters read auth"
  on public.academic_chapters for select to authenticated using (true);

create policy "academic_chapters admin write"
  on public.academic_chapters for all to authenticated
  using (public.has_role(auth.uid(), 'admin'))
  with check (public.has_role(auth.uid(), 'admin'));

create trigger academic_chapters_updated_at
  before update on public.academic_chapters
  for each row execute function public.tg_set_updated_at();
...
create policy "mcq_questions read auth"
  on public.mcq_questions for select to authenticated using (true);

create policy "mcq_questions admin write"
...
create policy "qbank_questions read auth"
  on public.qbank_questions for select to authenticated using (true);

create policy "qbank_questions admin write"
  on public.qbank_questions for all to authenticated
  using (public.has_role(auth.uid(), 'admin'))
  with check (public.has_role(auth.uid(), 'admin'));

create trigger qbank_questions_updated_at
...
create policy "mcq_attempts self all"
  on public.mcq_attempts for all to authenticated
  using (user_id = auth.uid())
  with check (user_id = auth.uid());

create policy "mcq_attempts admin select"
...
create policy "qbank_attempts self all"
  on public.qbank_attempts for all to authenticated
  using (user_id = auth.uid())
  with check (user_id = auth.uid());

create policy "qbank_attempts admin select"
  on public.qbank_attempts for select to authenticated
  using (public.has_role(auth.uid(), 'admin'));
...
create policy "wrong_answer_bookmarks self all"
  on public.wrong_answer_bookmarks for all to authenticated
  using (user_id = auth.uid())
  with check (user_id = auth.uid());
...
create policy "custom_exam_sessions self all"
  on public.custom_exam_sessions for all to authenticated
  using (user_id = auth.uid())
  with check (user_id = auth.uid());

create trigger custom_exam_sessions_updated_at
  before update on public.custom_exam_sessions
  for each row execute function public.tg_set_updated_at();
...
create policy "custom_exam_answers self all"
  on public.custom_exam_answers for all to authenticated
  using (user_id = auth.uid())
  with check (user_id = auth.uid());
...
create policy "routines self all"
...
create policy "routine_days self all"
...
create policy "routine_tasks self all"
  on public.routine_tasks for all to authenticated
  using (user_id = auth.uid())
  with check (user_id = auth.uid());

create trigger routine_tasks_updated_at
...
create policy "routine_task_completions self all"
  on public.routine_task_completions for all to authenticated
  using (user_id = auth.uid())
  with check (user_id = auth.uid());
...
create policy "student_preferences self all"
  on public.student_preferences for all to authenticated
  using (user_id = auth.uid())
  with check (user_id = auth.uid());
...
create policy "admin_settings read auth"
...
insert into public.admin_settings (settings) values ('{}'::jsonb)
on conflict (singleton) do nothing;

-- =========================================================================
-- Admin cross-user visibility for profiles
-- =========================================================================
